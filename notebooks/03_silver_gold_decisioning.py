...
CREATE OR REPLACE VIEW {CATALOG}.gold.vw_application_decisioning (
  ...
  risk_narrative          COMMENT 'One-line Bahasa risk narrative generated by ai_query (suspicious cases only).',
...
# MAGIC
# MAGIC Order by descending `risk_score` so the borderline cases come first. Include the `risk_narrative` so reviewers see *why*.
